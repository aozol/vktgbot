...
echo '<h1>Управление администраторами Шуткобота бота '.$_GET['vkGroupId'].'</h1>';
...
<div class="desc">
    <table><tr><th>Текущий админ (vk id)</th><th>Действие</th></tr>';
    
    while (list($vkId)=mysqli_fetch_array($sql))
    {   
        $user_info=vkApi_usersGet($vkId);
        echo '<tr><td>'.$vkId.'</td><td><select name="action['.$vkId.']">'.$action_list.'</select></td></tr>';
    }
...
<div class="desc">
<textarea name="newAdmin" style="width: 200px; height: 100px"></textarea>
</div>
<div class="bottom"><div></div></div>
</div> 
</div></div>
