...
if (($incommingText=='Начать') OR ($incommingText=='Start') OR ($incommingText=='начать') OR ($incommingText=='start') )
        {
          ...
          mysqli_query($dblink,"INSERT INTO `".DBP_GENERAL."userInfo` (vkId,sex,first_name,last_name,username) VALUES ({$vkId},0,'{$data['chat']['first_name']}','{$data['chat']['last_name']}','{$data['chat']['username']}')");
        }
        ...
        mysqli_query($dblink,"UPDATE `".DBP_GENERAL."userInfo` SET first_name='{$data['chat']['first_name']}', last_name='{$data['chat']['last_name']}', username='{$data['chat']['username']}' WHERE vkId={$vkId}");
        ...
        //log_msg('payloadText: '.$payloadText);
...
Дерзай! Нажми "Добить" прямо сейчас!';
          ...
          case 'shutkobot_my_edit':
            ...
            if(!$incommingText)
            {
                ...
                $replyText="Для того, чтобы изменить свою добивку, отправь сообщение с текстом \"[номер добивки]::[новый текст]\" (два двоеточия обязательны, кавычки не нужны).
                
Твои добивки на зачин \"{$startText}\" (с их номерами):";
...
Если на самом деле ты хочешь предложить смешное завершение (добивку), то для этого надо НАЖАТЬ КНОПКУ "ДОБИТЬ"!';
          ...
          case 'shutkobot_vote':
            ...
            if (stristr($payloadParams,',')) //требуется поправить!!!
            {
                ...
                list($vote,$finishId)=explode(',',$payloadParams);
                mysqli_query($dblink,"INSERT INTO `".DBP."votes` (startId,finishId,vkId,vote) VALUES ({$startId},{$finishId},{$vkId},{$vote})");
...
Нажми кнопку с оценкой 1, 3, 5, 8 ли 13!";

                $buttons[0][0][0]["payload"]="shutkobot_vote:1,".$finishId;
                $buttons[0][0][1]='1';
                $buttons[0][0][2]='positive';

                $buttons[0][1][0]["payload"]="shutkobot_vote:3,".$finishId;
                $buttons[0][1][1]='3';
                $buttons[0][1][2]='positive';

                $buttons[0][2][0]["payload"]="shutkobot_vote:5,".$finishId;
                $buttons[0][2][1]='5';
                $buttons[0][2][2]='positive';

                $buttons[0][3][0]["payload"]="shutkobot_vote:8,".$finishId;
                $buttons[0][3][1]='8';
                $buttons[0][3][2]='positive';

                $buttons[0][4][0]["payload"]="shutkobot_vote:13,".$finishId;
                $buttons[0][4][1]='13';
                $buttons[0][4][2]='positive';
            }
            //*/
          ...
          case 'shutkobot_share':
          	...
          	$text="Приглашаю присоединиться к Шуткоботу \"{$botInfo[$currentGroupId]['name']}\"!

Там можно пошутить самому, а также почитать и оценить шутки других участников.

Переходи по ссылке и нажимай \"Начать\":";
        if (isset ($botInfo[$currentGroupId]['tg'])) if ($botInfo[$currentGroupId]['tg'])
        $text.="
Telegram: https://t.me/{$botInfo[$currentGroupId]['tg']}";
if (isset ($botInfo[$currentGroupId]['vk'])) if ($botInfo[$currentGroupId]['vk'])
        $text.="
VK: https://vk.me/{$botInfo[$currentGroupId]['vk']}";
		...
		vkApi_messagesSend($vkId, 'Чтобы пригласить друзей в Шуткобота, перешли сообщение ниже. Также можешь разместить его постом на своей странице.');
          ...
          case 'shutkobot_create':
          	...
          	$text="На данный момент автоматическое создание шуткоботов для телеграмма не поддерживается.";
...
Для ответа перейдите по ссылке: https://'.BOT_HOST.'adm/?f=dialog&vkId='.$vkId);
               }
            }
            else
            {
