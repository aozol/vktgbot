...
if (!isset($_POST['listId'])){
   ...
   $text='';
    ...
    if(!isset($dataArray['vkGroupId']))
    {
        $dataArray['vkGroupId']=0;
        $dataArray['listId']=array();
        $dataArray['listIdExclude']=array();
        $dataArray['dateTime']='';
        $dataArray['text']='';
        $dataArray['buttons_php']='';
    }
    if ($adm_info['root'])
   ...
   while (list($vkGroupId)=mysqli_fetch_array($sql))
   {
    ...
    if ($adm_info['root'])
        $sql=mysqli_query($dblink,"SELECT e.eventId,e.eventName,er.roleId,er.roleName FROM `".DBP."events` e JOIN `".DBP."events_roles` er ON e.eventId=er.eventId");
    else
        $sql=mysqli_query($dblink,"SELECT e.eventId,e.eventName,er.roleId,er.roleName FROM `".DBP."events` e JOIN `".DBP."events_roles` er ON e.eventId=er.eventId WHERE e.vkGroupId IN (SELECT vkGroupId FROM `".DBP."vkApi` WHERE admin={$adm_info['id']})");
        


    while (list($eventId,$eventName,$roleId,$roleName)=mysqli_fetch_array($sql))
    {
        $eventRoleName["{$eventId},{$roleId}"]="{$eventName} | {$roleName}";
    ...
    foreach ($eventRoleName as $k=>$v)
    {
    
        if(in_array ($k, $dataArray['eventsRolesId']))
            $eventsRoles.='<option value="'.$k.'" selected>'.$v;
        else
            $eventsRoles.='<option value="'.$k.'">'.$v;
        
        if(in_array ($k, $dataArray['eventsRolesIdExclude']))
            $eventsRolesExclude.='<option value="'.$k.'" selected>'.$v;
        else
            $eventsRolesExclude.='<option value="'.$k.'">'.$v;        
    
    
    }
    
    $type='<option value="mlists">Учитывать только списки рассылки
    <option value="events">Учитывать только мероприятия
    <option value="and">Учитывать оба условия одновременно ("И")
    <option value="or">Учитывать выполнение любого условия ("ИЛИ")';
    
    $type=str_replace("value=\"{$dataArray['type']}\"","value=\"{$dataArray['type']}\" selected",$type);
...
<h2>Тип рассылки</h2>
<p><select name="type">'.$type.'</select><br/><br/></p>
<h2>Списки рассылок</h2>
<p><br/><br/>
...
<h2>Мероприятия</h2>
<p>Отправить (cписок): <select name="eventsRolesId[]" multiple>'.$eventsRolesExclude.'</select><br/>
Дополнительные Vkid через запятую: <input type="text" name="vkIds" value=""></p>
<p>Исключить получателей, которые также подписаны на списки:<br/><br/>
Исключить (cписок): <select name="eventsRolesIdExclude[]" multiple>'.$eventsRolesExclude.'</select><br/>
Дополнительные исключаемые Vkid через запятую: <input type="text" name="vkIdsExclude" value=""></p>
