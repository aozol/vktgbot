...
while (list($taskId,$dataJson)=mysqli_fetch_array($sql))
{
        ...
        if($task_users)
            echo "<tr><td>{$dataArray['dateTime']}</td><td>{$dataArray['text']}</td><td><a href=\"?f=message_to_list&taskId={$taskId}\">Изменить</a><br><br><a href=\"?f=message_to_list&taskId={$taskId}&copy=1\">Копировать</a><br><br><a href=\"?f=message_tasks_users&taskId={$taskId}\">Список получателей</a></td></tr>";
        else
