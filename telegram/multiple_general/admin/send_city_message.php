...
$page_title='Отправка сообщения подписчикам по городам';

require_once(dirname(__FILE__).'/template/top.php');

echo '<h1>Отправка сообщения подписчикам по городам</h1>';
...
$sql=mysqli_query($dblink,"SELECT cityId,cityName,countryName FROM `city_kit_cities` as cities LEFT JOIN `city_kit_countries` as countries ON cities.`countryId`=countries.`countryId` ORDER BY countryName,cityName");

//*
while (list($cityId,$cityName,$countryName)=mysqli_fetch_array($sql))
{
    $cityNames[$cityId]="{$countryName} - {$cityName}";
}
...
if (!isset($_POST['listId'])){
   ...
   if ($_GET['taskId'])
    ...
    foreach ($cityNames as $k=>$v)
    {
        ...
        if(in_array ($k, $dataArray['cityId']))
            $citiesList.='<option value="'.$k.'" selected>'.$v;
        else
            $citiesList.='<option value="'.$k.'">'.$v;        
    ...
    if($vkGroupIds)
    {
        ...
        $result=explode(',',$vkGroupIds);

        foreach($result as $k=> $v)
        {
            if($vkGroupId==$dataArray['vkGroupId'])
                $groupsList.= "<option value=\"{$v}\" selected>{$v}";
            else
                $groupsList.= "<option value=\"{$v}\">{$v}";
...
<div class="col w2"><div class="content">
<div class="box header">
<div class="head"><div></div></div>
<h2>Отправить получателям из городов</h2>
<div class="desc">
<select name="cityId[]" multiple>'.$citiesList.'</select>
</div>
<div class="bottom"><div></div></div>
</div>
</div></div>
...
<div class="box header">
<div class="head"><div></div></div>
<h2>Которые в списках получателей</h2>
<div class="desc">
