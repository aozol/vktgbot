...
if(isset($_POST['vkGroupId']))
{
    ...
    header('Location: https://oauth.vk.com/authorize?client_id='.VK_APP_ID.'&display=page&redirect_uri=https://'.BOT_HOST.'admin/?f=install_group&group_ids='.$gId.'&scope=manage&response_type=code&v=5.80');
...
elseif(isset($_GET['code']))
{
    ...
    $token_arr=json_decode(file_get_contents('https://oauth.vk.com/access_token?client_id='.VK_APP_ID.'&redirect_uri=https://'.BOT_HOST.'admin/?f=install_group&client_secret='.VK_CLIENT_SECRET.'&code='.$_GET['code']),TRUE);
    ...
    if(list($confirmationToken,$secretKey)=mysqli_fetch_array($sql)) // если группа уже добавлена - завершаем настройку
    {
        mysqli_query($dblink,"UPDATE `".DBP."vkApi` SET token='{$token}' WHERE vkGroupId={$_COOKIE['vkGroupId']}");
            ...
            <p>Теперь вы можете перейти в <a href=\"../admin\">панель администратора</a></p>";
    ...
    mysqli_query($dblink,"INSERT INTO `".DBP."vkAdmin` (vkGroupId,vkId) VALUES ({$_COOKIE['vkGroupId']},{$_COOKIE['vkId']})");
...
elseif(isset($_POST['getMessages']))
{
    ...
    header('Location: https://oauth.vk.com/authorize?client_id='.VK_APP_ID.'&display=page&redirect_uri=https://'.BOT_HOST.'admin/?f=install_group&group_ids='.$_COOKIE['vkGroupId'].'&scope=messages&response_type=code&v=5.80');
