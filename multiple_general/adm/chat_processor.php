...
- '.$mlistName[$mlistId];
                   ...
                   $unsub_buttons[$us/2][$us%2][0]["payload"]="unsub:".$mlistId;
...
Для управления подпиской используйте кнопки ниже.';
          ...
          case "unsub":
             ...
             if(user_mlist_manage($vkId,$payloadParams,0))
             	...
             	$text='Не удалось отменить подписку на рассылку: "'.$mlistName[$payloadParams].'", т.к. Вы подписывались на нее через другое сообщество.';
             vkApi_messagesSendButtons($vkId, $text,array_merge($buttons,$buttons_def));
