...
if (isset ($_POST['vkGroupId']))  //Обработчик сохранения формы
{
   ...
   foreach ($vkIds as $vkId)
   {
        ...
        $json='{"type":"message_new","object": {"message": {"date":0,"from_id":'.$vkId.',"id":0,"out":0,"peer_id":'.$vkId.',"text":"'.$_POST['incommingText'].'","conversation_message_id":0,"fwd_messages":[],"important":false,"random_id":0,"attachments":[]';
        
        if ($_POST['payloadText'])
            $json.=',"payload":"{\"payload\":\"'.$_POST['payloadText'].'\"}"';
        
        $json.=',"is_hidden":false},"client_info":{"button_actions":["text","vkpay","open_app","location","open_link","callback","intent_subscribe","intent_unsubscribe"],"keyboard":true,"inline_keyboard":true,"carousel":true,"lang_id":0}},"group_id":'.$_POST['vkGroupId'].',"secret":"'.$secret.'"}';
        
        $query=http_build_query([$json]);
        $url = 'https://'.BOT_HOST.'chat_processor.php?emulate=1';
