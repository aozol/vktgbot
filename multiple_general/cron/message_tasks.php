...
require_once(dirname(__FILE__).'/../adm/template/top_min.php');
...
while (list($taskId,$ADM_VK_ID,$DBP,$dataJson)=mysqli_fetch_array($sql))
{
  ...
  $vkGroupId=array();
  while (list($vkId,$gId,$sex)=mysqli_fetch_array($sql2))
  {
    $id_arr[]=$vkId;
    $vkGroupId[$vkId]=$gId;
    $vkSex[$vkId]=$gId;
...
mysqli_query($dblink,"DELETE FROM `".DBP_GENERAL."message_queue` WHERE dateTime<='".date("Y-m-d H:i:s",time()-3600*24*90)."'");
            

//echo file_get_contents('http://vkbot.clubevrika.ru/multiple_general/adm/message_sender.php?from_tasks=1');


require_once(dirname(__FILE__).'/../adm/template/bottom_min.php');
?>
