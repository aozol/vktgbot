...
$sql=mysqli_query($dblink,"SELECT GROUP_CONCAT(distinct vkId) as vkIds, token, messageText, keyboard FROM `".DBP_GENERAL."message_queue`  WHERE dateTime<='".date("Y-m-d H:i:s")."' AND sended=0 GROUP BY token,messageText,keyboard LIMIT 0,100");
...
while (list($vkIds, $token, $messageText, $keyboard)=mysqli_fetch_array($sql))
{
    ...
    foreach ($vkIds_arr as $vkIds)
    {
        ...
        if(isset($buttons))
            ...
            $res=vkApi_messagesSend($vkIds, $messageText, $attachments);
            
        
        
        
        
        mysqli_query($dblink,"UPDATE`".DBP_GENERAL."message_queue` SET sended=1  WHERE dateTime<='".date("Y-m-d H:i:s")."' AND vkId IN ({$vkIds}) AND messageText='".mysqli_real_escape_string($dblink,$messageText)."' AND token='{$token}'");
