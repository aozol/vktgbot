...
require_once(dirname(__FILE__).'/API/VK.php');
...
function user_mlist_manage($vkId,$mlistId,$action=1,$gId=0)
{
  ...
  switch($action)
  {
    ...
    case 2: //подписка на рассылку по умолчанию для текущей группы
        ...
        if(!mysqli_num_rows($sql))
            vkApi_messagesSend(ADM_VK_ID,"Ошибка бота: список по умолчанию не опеределен. Укажите один из списков рассылки как список по умолчанию, чтобы ошибка не повторялась.");
...
function bot_debugger($message)
{
    ...
    if(defined('ADM_VK_ID'))
        $ADM_VK_ID=ADM_VK_ID;
    else
        $ADM_VK_ID=$GLOBALS['ADM_VK_ID'];
